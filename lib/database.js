...
class Db {
  ...
  _toText(action, json) {
    ...
    const mustPrefix = action && action !== 'save'
    return mustPrefix ? action + '|' + text : text
  }
  ...
  async _compactDB() {
    ...
    for (let r = 0; r < lines.length; r++) {
      ...
      const a = line.substr(0, index)
      let b = line.substring(index + 1)
      if (a.includes('"')) b = line // not a separator but inside a string {"message":"44B | CC8"}
      if (!a) tempSet.add(line) // 'save' is implicit when no action is set
      else if (a === 'delete') tempSet.delete(b)
      else tempSet.add(b)
    }
