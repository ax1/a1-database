...
class Db {
  ...
  async _compactDB() {
    ...
    try {
      ...
      if (err.code === 'ENOENT') await fswriteFile(this._filePath, content)
      else throw (err)
    }
    const lines = content.split('\n').filter(el => el !== '')
    const clean = lines.filter((el, i) => el !== '' && lines.lastIndexOf(el) === i) //duplicated strings are removed
    clean.map(el => el.split('|'))
      .forEach(item => {
        ...
        switch (action) {
          case 'save':
            ...
            break
          case 'delete':
            arr.filter(el => el === text)
              .forEach(el => { const index = arr.indexOf(el); return arr.splice(index, 1) })
            break
          default:
            throw new Error(action + ' is not allowed when compacting')
        }
      })
