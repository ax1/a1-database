...
class Db {
   ...
   * 
   * This method performs either find(id) or find(filter)
   * 
   * @param {number|String|Function} id_or_filter - if number or String it finds by  the 'id' in database,
   * @returns Array[Object]
   */
  async find(id_or_filter) {
    const filter = typeof id_or_filter === 'function' ? id_or_filter : el => el.id === id_or_filter
 ...
 */
  ...
  async _loadFile() {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        try { await fs.mkdir(path.dirname(this._filePath)) } catch (e) { console.log('folder already exists') }
